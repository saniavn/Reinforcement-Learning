...
from env_large import animate
import h5py
START = (39,10)  # start state
GOAL = (8,8)  # goal state
# Actions
RIGHT = 0
LEFT = 1
...
def states(s):
    if map[tuple(s)]== 4 or map[tuple(s)]== 1:
        ...
        return a
    # if map[tuple(s)]==3 or map[tuple(s)]==2 :
    #     return (0,0)
    else:
        return (0,0)
...
if __name__=="__main__":
    ...
    # plt.subplot(2, 1, 1)
    # plt.plot(np.mean(np.asarray(r), axis=0), color='red')
    # plt.plot(range(1, NUM_EPISODES + 1), steps, label='DQ_learning', color='blue')
    # plt.xlabel('episodes')
    ...
    # plt.subplot(2, 1, 2)
    #
    # plt.plot(np.mean(np.asarray(steps), axis=0), color='red')
    # plt.plot(range(1, NUM_EPISODES + 1), np.cumsum(r), label='Double Q-learning', color='blue')
    # plt.xlabel('episodes')
